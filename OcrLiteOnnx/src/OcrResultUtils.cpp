...
OcrResultUtils::OcrResultUtils(JNIEnv *env, OcrResult &ocrResult) {
    ...
    jclass jOcrResultClass = env->FindClass("li/murphy/pojo/OcrResult");
...
jobject OcrResultUtils::newJPoint(cv::Point &point) {
    jclass clazz = jniEnv->FindClass("li/murphy/pojo/Point");
    if (clazz == NULL) {
...
jobject OcrResultUtils::getTextBlock(TextBlock &textBlock) {
    ...
    jclass clazz = jniEnv->FindClass("li/murphy/pojo/TextBlock");
    if (clazz == NULL) {
